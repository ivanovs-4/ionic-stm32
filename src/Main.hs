...
Copied from https://github.com/HaskellEmbedded/ion/blob/master/src/Ivory/Language/Ion/Examples/Example.hs
-}
...
main = join $ execParser $ info (opts <**> helper)
        ( fullDesc
       -- <> progDesc "Apply trained model to historic data to generate continuation"
       -- <> header "Toy apply"
        )
  where
    opts :: Parser (IO ())
...
test = ion "Foo" $ do
  ...
  cond (pure false) $ ion "condTest" $ do
